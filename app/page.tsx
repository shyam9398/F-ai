...
export default function Home() {
  ...
  const [activeDiscover, setActiveDiscover] = useState("Trending Papers");
  ...
  // 2. Discover Category filter logic (from left sidebar)
  ...
  // 3. Sorting logic (Citations, Newest, stars)
  const processedPapers = useMemo(() => {
    const papersCopy = [...categoryFilteredPapers];
    
    // Left sidebar sorting override
    if (activeDiscover === "Latest Papers") {
      return papersCopy.sort((a, b) => b.year - a.year);
    } else if (activeDiscover === "Most GitHub Stars") {
      // Sort by whether it has a github repo first
      return papersCopy.sort((a, b) => (b.github_url ? 1 : 0) - (a.github_url ? 1 : 0) || b.citations - a.citations);
    }

    // Horizontal filter sorting
    ...
    return papersCopy;
  }, [categoryFilteredPapers, currentSort, activeDiscover]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 flex w-full">
        {/* Left Sidebar Menu */}
        <Sidebar activeDiscover={activeDiscover} onDiscoverChange={setActiveDiscover} />

        {/* Main Content Area */}
        <main className="flex-1 min-w-0 px-8 py-6 bg-white overflow-x-hidden">
          {selectedPaper ? (
